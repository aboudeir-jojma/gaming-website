...
export default function GamePage({ game }) {
  ...
  // 🔁 Reset started when navigating to a different game
  useEffect(() => {
    setStarted(false);
  }, [router.asPath]);
  ...
  const toggleFullscreen = async () => {
    ...
    if (!document.fullscreenElement) {
      try {
        ...
        gameContainer.classList.add("desktop-fullscreen");
        setIsFullscreen(true);
      } catch (err) {
      ...
      try {
        ...
        gameContainer.classList.remove("desktop-fullscreen");
        setIsFullscreen(false);
      } catch (err) {
        ...
        <div className="flex-1">
          {!isFullscreen && (
            <Link
              href="/"
              locale={locale}
              className="inline-flex items-center gap-2 px-3 py-2 rounded-lg bg-gradient-to-br from-indigo-500 to-purple-600 text-white shadow-md transition duration-200 hover:brightness-110"
              aria-label={t("back")}
            >
          ...
          {!isFullscreen && (
            <>
              <div className="mt-4 flex items-center gap-3 text-white/70">
                ...
                <div className="text-sm">{t("categorystug")}: {categoryText}</div>
              </div>
