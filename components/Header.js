...
// ...existing code...
  return (
    <header className="sticky top-0 z-30 w-full border-b border-white/5 bg-white text-black dark:bg-[#12131a]/80 dark:text-white sm:backdrop-blur transition-colors duration-300">
...
>
  ☰
        ...
        <Link href="/" className="shrink-0 text-base sm:text-lg font-extrabold tracking-tight text-black dark:text-white">
          PcGameOn
        </Link>
...
<button
  onClick={toggleTheme}
  className="ml-2 shrink-0 rounded-xl px-3 py-2 text-sm
             bg-transparent text-black dark:text-white
             hover:bg-black/5 dark:hover:bg-white/10
             transition-colors duration-200"
>
  ...
  <div className="flex items-center sm:hidden">
    <Star className={`w-5 h-5 ${rating > 0 ? "fill-yellow-400 text-yellow-400" : "text-zinc-400"}`} />
  </div>
  <div className="hidden sm:flex items-center gap-1" aria-label="Rate this site from 1 to 5 stars">
    {[1,2,3,4,5].map(i => {
      ...
      return (
        ...
        >
          <Star className={`w-5 h-5 transition-transform ${active ? "fill-yellow-400 text-yellow-400" : "text-zinc-400"}`} />
        </button>
      );
    })}
  </div>
